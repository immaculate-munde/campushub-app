...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    const intervalId = setInterval(fetchQuote, 10000); // Refresh every 10s
  ...
  return (
    <motion.div variants={containerVariants} initial="hidden" animate="visible" className="grid grid-cols-1 md:grid-cols-2 gap-6">
      ...
      {/* Quick Links */}
      <motion.div variants={itemVariants} className={`col-span-1 md:col-span-2 p-6 rounded-xl shadow-md ${theme === 'dark' ? 'bg-gray-800' : 'bg-white'}`}>
        <h2 className={`text-lg font-semibold mb-4 ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>Quick Access</h2>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {quickLinks.map((link, index) => (
            <Link
              key={index}
              to={link.path}
              className={`flex items-center p-4 rounded-lg shadow-sm transition-transform duration-200 transform hover:scale-105 ${
